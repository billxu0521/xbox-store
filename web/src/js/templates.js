...
<section>
  ...
  <a class="see-all link" id="collection-${section.type}" href="/collection/${section.type}" aria-label="Ver el listado de ${section.title}">更多</a>
</section>
`);
}
...
<div class="game-price">
  ...
  <span class="game-price-amount">
    ${(game.price.deal || game.price.amount) ?
      `<x-price amount="${game.price.deal || game.price.amount}"></x-price>`
      : game.demo ? 'Demo' : 'Gratis'
    }
  </span>
  ${game.price.deal ? `<div class="game-price-prev">
    <x-price amount="${game.price.amount}" strike></x-price>
  </div>` : ''}
  ${game.price.amount > 0 ?
    `<small class="game-price-taxes">含稅</small>`
    : ''
  }
  ${game.gold_deal ? `<div>Precio Gold: <x-price amount="${game.price.gold_deal}"></x-price></div>` : ''}
</div>
  `);
}
...
<article class="game-preview" style="--game-preview-url: url(${img}?w=1000&q=70)">
  ...
  <div>
    <div class="game-preview-info">
      ...
      ${gamePriceTemplate(game)}
      <a href="https://www.xbox.com/zh-tw/games/store/a/${game.id}" class="game-buy-now btn">立即購買</a>
      ${until ? `<div class="game-deal-ends"><small>優惠還有 ${until} 天</small></div>` : ''}

      <h4>發布日期</h4>
      <time datetime="${new Date(game.release_date).toLocaleString('zh-TW')}">${new Date(game.release_date).toLocaleString('zh-TW', { day: '2-digit' , month: '2-digit', year: 'numeric',  })}</time>
...
<section class="gamepass">
  ...
  <ul>
    <li><a href="/gamepass/new" id="gamepass-new" class="link">新加入</a></li>
    <li><a href="/gamepass/coming" id="gamepass-coming" class="link">即將加入</a></li>
    <li><a href="/gamepass/leaving" id="gamepass-leaving" class="link">即將離開</a></li>
    <li><a href="/gamepass/all" id="gamepass-all" class="link">瀏覽全部</a></li>
  </ul>
</section>
  `);
}
...
<section class="gold">
  ...
  <ul>
    <li><a href="/gold/gold-new" id="gold-new" class="link">可用</a></li>
    <li><a href="/gold/gold-deals" id="gold-deals" class="link">優惠</a></li>
    <li><a href="/gold/gold-free" id="gold-free" class="link">Gold會員免費</a></li>
  </ul>
</section>
  `);
}
